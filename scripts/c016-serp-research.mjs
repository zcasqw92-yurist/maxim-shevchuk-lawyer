...
const outputDir = join(root, "reports", "c016-serp-research");
const query = "как взыскать оплату за выполненную работу без договора";
...
const searchYandex = async () => {
  ...
  const body = {
    ...
    userAgent: "Mozilla/5.0 (compatible; C016Research/1.0)",
  };
...
const toMarkdown = (report) => {
  const lines = [
    "# C-016 — слепок органической выдачи",
    "",
...
console.log(`C-016 SERP: Yandex=${yandexCount}, Google=${googleCount}, errors=${errors.length}`);
