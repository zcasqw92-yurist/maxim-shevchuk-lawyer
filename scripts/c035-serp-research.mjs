...
const outputDir = join(root, "reports", "c035-serp-research");
const query = "некачественно покрасили машину в автосервисе что делать";
...
const searchYandex = async () => {
  ...
  const body = {
    ...
    userAgent: "Mozilla/5.0 (compatible; C035Research/1.0)",
  };
...
const toMarkdown = (report) => {
  const lines = [
    "# C-035 — слепок органической выдачи",
    "",
...
console.log(`C-035 SERP: Yandex=${yandexCount}, Google=${googleCount}, errors=${errors.length}`);
