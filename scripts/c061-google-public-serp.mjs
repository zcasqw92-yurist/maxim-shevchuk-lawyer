...
const attemptDefinitions = (queryText) => {
  const params = (extra = {}) => new URLSearchParams({ q: queryText, num: "10", hl: "ru", gl: "ru", pws: "0", filter: "0", ...extra }).toString();
  return [
...
const extractResults = async (page) => page.evaluate((labels) => {
  ...
  const sponsored = [];
  const seen = new Set();
  const headings = Array.from(document.querySelectorAll("#search h3, #rso h3, #tads h3, [data-text-ad] [role='heading'], main h3"));
  ...
  return { organic, sponsored, diagnostics: { headingCount: headings.length, adContainerCount: document.querySelectorAll("#tads,[data-text-ad]").length } };
}, sponsoredLabels);
  ...
  for (const [index, attempt] of attemptDefinitions(queryItem.query).entries()) {
    const context = await browser.newContext({
      ...
      userAgent: attempt.mobile
        ? "Mozilla/5.0 (Linux; Android 14; Pixel 8) AppleWebKit/537.36 Chrome/150 Mobile Safari/537.36"
        : "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/150 Safari/537.36",
      extraHTTPHeaders: { "Accept-Language": "ru-RU,ru;q=0.9,en;q=0.7", DNT: "1" }
    });
    const page = await context.newPage();
    try {
      const response = await page.goto(attempt.url, { waitUntil: "domcontentloaded", timeout: 15000 });
      await page.waitForTimeout(500);
      ...
      attempts.push({ attempt: index + 1, requestedUrl: attempt.url, finalUrl: page.url(), httpStatus: response?.status() || null, blocked, organicCount: organic.length, sponsoredCount: sponsored.length });
  ...
  return {
    ...
    diagnostics: {
      reason: attempts.some((item) => item.blocked)
        ? "Google returned a robot or unusual-traffic challenge for all official Google domains and modes"
...
const report = {
  ...
  source: "Google public SERP via official Google domains and result modes",
...
try {
  for (const queryItem of config.queries) {
    ...
    console.log(`Google public ${queryItem.id}: organic=${result.organicResultsReviewed}, sponsored=${result.sponsoredResultsObserved}, status=${result.status}`);
...
console.log(`Google public gate passed: ${report.gatePassed}`);
