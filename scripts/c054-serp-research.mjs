...
const outputDir = join(root, "reports", "c054-serp-research");
const query = "узнал о судебном приказе после списания денег что делать";
...
const searchYandex = async () => {
  ...
  const body = {
    ...
    userAgent: "Mozilla/5.0 (compatible; C054Research/1.0)",
  };
...
const toMarkdown = (report) => {
  const lines = [
    "# C-054 — слепок органической выдачи",
    "",
...
console.log(`C-054 SERP: Yandex=${yandexCount}, Google=${googleCount}, errors=${errors.length}`);
