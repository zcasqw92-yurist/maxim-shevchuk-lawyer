...
Этот документ разбирает известные неудачные или ложно-красные публикационные циклы проекта и фиксирует правило: **историческая ошибка считается закрытой только тогда, когда её причина представлена машинным regression-контролем либо безопасным ограниченным recovery-маршрутом**.

Машинный источник: `config/publication-failure-regressions.json`. Блокирующий тест: `scripts/publication-readiness-test.mjs`.
...
**Постоянная защита:** полный `npm run check` проходит в PR; deterministic `release-check.mjs` проходит до браузеров и повторяется при release. Новый `test:publication-readiness` контролирует сам release-контур. Если падает project-code test, автоматический recovery запрещён: исправление делается в ветке и снова проходит PR.
...
**Постоянная защита:** `npm run check` до merge обходит все публикации в Chromium/WebKit на контрольных узких viewport; `test:cross-browser` и `test:all-publications-overflow` обязательны. Запрещено лечить регрессию повышением tolerance или глобальным `overflow-x:hidden`.
...
`actions/deploy-pages@v4` создавал deployment, но GitHub Pages backend держал его в очереди дольше стандартного ожидания. Build и artifact были исправны, а action завершал ожидание по timeout.
...
Историческая схема допускала `cancel-in-progress:true`, отдельный macOS fallback и собственный Pages REST client. Новый push мог пересечься с уже созданным server-side deployment, а fallback — создать второй writer в тот же backend.

**Неисправность:** одним production resource управляли несколько механизмов с разными очередями и polling.

**Постоянная защита:** единственный writer — `Deploy GitHub Pages` с `actions/deploy-pages@v5`; `cancel-in-progress:false`, `queue:single`. Recovery/watchdog имеют только `actions:write` и могут rerun/dispatch этот workflow, но не имеют Pages permissions и не содержат deploy actions.
...
**Постоянная защита:** `service-pages-interaction-test.mjs` использует Sparticuz binary только при `process.platform === "linux"`; `browser-test-contract.mjs` запрещает unconditional Linux executablePath. Отдельный browser-heavy deployment fallback удалён, поэтому cross-platform browser test больше не является recovery-путём Pages.
...
После успешного Pages deployment отдельный verification повторно скачивал Chromium/WebKit и мог стать красным из-за apt/CDN/Playwright, хотя production уже был обновлён.
...
**Постоянная защита:** post-deploy передаёт `INDEXNOW_SITEMAP_URL=https://yuristshevchuk.com/sitemap.xml`; script валидирует origin и читает уже проверенный production sitemap. Syntax всех post-deploy scripts проверяется до merge.
...
**Постоянная защита:** новые контракты проверяют свойства: permissions, единственность writer, порядок шагов, наличие exact-SHA gate, отсутствие браузеров post-deploy, bounded recovery. Где важен порядок, он проверяется индексами смысловых маркеров, а не многострочным хрупким regex.
...
`fetch-depth:0` использовался ради `BASE_SHA..HEAD`, хотя репозиторий содержит большое количество исторических `agent/*` branches. Это раздувало checkout/log/network и увеличивало инфраструктурную поверхность сбоя.

**Неисправность:** лишняя Git history dependency.

**Постоянная защита:** PR CI и Pages build используют `fetch-depth:1` и точечно fetch только `CONTENT_GOVERNANCE_BASE_SHA`. Post-deploy также обязан checkout exact deployed SHA с depth 1.
...
`branch → publication readiness → full PR CI → merge → one official Pages deploy → exact SHA marker → production HTTP regression → IndexNow`.
...
- **setup-only hosted runner failure** — до двух автоматических повторов failed jobs того же run/SHA;
- **Pages/backend failure после запуска project code** — не маскировать retry-коммитом; тот же штатный workflow может быть повторно вызван watchdog только при доказанном production drift;
...
- **Metrica/API telemetry failure** — диагностическая, не должна менять статус опубликованного сайта.

# Критерий завершённой публикации

Публикация считается завершённой только если:

1. точный head SHA прошёл обязательный PR CI;
2. в `main` попал именно проверенный head;
3. официальный Pages workflow создал deployment без альтернативного writer;
4. `https://yuristshevchuk.com/deployments/<SHA>.json` существует и содержит тот же SHA;
5. `build-info.json` и `<meta name="site-build-sha">` совпадают с SHA;
6. production HTTP regression всех публикаций прошёл;
7. public-copy regression прошёл;
8. только после этого выполняется IndexNow.

Ни зелёный merge, ни успешный build, ни наличие нового commit в `main` сами по себе не означают, что production обновлён.
