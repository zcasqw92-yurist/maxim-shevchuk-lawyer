...
const outputDir = join(root, "reports", "c003-serp-research");
...
const searchYandex = async () => {
  ...
  const body = {
    ...
    userAgent: "Mozilla/5.0 (compatible; C003Research/1.0)",
  };
  ...
  const organic = uniqueOrganic(docs).slice(0, 20);
  return {
    engine: "Yandex",
...
const searchGoogle = async () => {
  ...
  if (serpApiKey) {
    ...
    const payload = await fetchJson(`https://serpapi.com/search.json?${params}`, {}, "SerpApi Google");
    const organic = uniqueOrganic((payload.organic_results || []).map((item) => ({
      position: Number(item.position || 0),
      title: clean(item.title),
      link: String(item.link || ""),
      snippet: clean(item.snippet),
    }))).slice(0, 20);
    const sponsored = (payload.ads || []).map((item) => ({
      position: Number(item.position || 0),
      title: clean(item.title),
      link: String(item.link || ""),
      label: "Ad",
    }));
    return { engine: "Google", provider: "SerpApi", organic, sponsored };
  }

  if (serperKey) {
    console.log(`::add-mask::${serperKey}`);
    const payload = await fetchJson(
    ...
    const organic = uniqueOrganic((payload.organic || []).map((item) => ({
      position: Number(item.position || 0),
      title: clean(item.title),
      link: String(item.link || ""),
      snippet: clean(item.snippet),
    ...
    return { engine: "Google", provider: "Serper", organic, sponsored };
  }

  throw new Error("Google: отсутствуют SERPAPI_API_KEY и SERPER_API_KEY");
};
...
const main = async () => {
  ...
  console.log(`C-003 SERP: Yandex=${yandexCount}, Google=${googleCount}, errors=${errors.length}`);
